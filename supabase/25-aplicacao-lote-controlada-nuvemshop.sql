...
begin
  ...
  if not found
     ...
     or v_conexao.limite_aplicacao not between 1 and 3
...
begin
  ...
  if p_limite not between 2 and 3 then
    raise exception 'O lote controlado deve conter dois ou tres itens.';
  end if;
...
begin
  ...
  if p_chave_operacao is null
     ...
     or v_total not between 2 and 3
     or v_total_distinto <> v_total then
    raise exception 'O lote exige dois ou tres itens distintos.';
  end if;
...
begin
  ...
  if not found
     ...
     or v_aplicacao.total_itens not between 2 and 3
...
begin
  ...
  where id = p_aplicacao_id
    and modo = 'aplicacao'
    and total_itens between 2 and 3
