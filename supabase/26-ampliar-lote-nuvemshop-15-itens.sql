...
begin
  ...
  if not found
     ...
     or v_conexao.limite_aplicacao not between 1 and 15
...
begin
  ...
  if p_limite not between 2 and 15 then
    raise exception 'O lote controlado deve conter de dois a quinze itens.';
  end if;
...
begin
  ...
  if p_chave_operacao is null
     ...
     or v_total not between 2 and 15
     or v_total_distinto <> v_total then
    raise exception 'O lote exige de dois a quinze itens distintos.';
  end if;
...
begin
  ...
  if not found
     ...
     or v_aplicacao.total_itens not between 2 and 15
...
begin
  ...
  where id = p_aplicacao_id
    and modo = 'aplicacao'
    and total_itens between 2 and 15
