  ...
  try {
    ...
    if (operationMode === "habilitar_lote") {
      ...
      if (
        ...
        return jsonResponse({
          error: "Selecione de dois a cinco itens da simulacao recente.",
          escrita_executada: false,
        }, 400, headers);
      }
    ...
    if (operationMode === "aplicar_lote") {
      ...
      if (
        ...
        return jsonResponse({
          error: "Selecione de dois a cinco itens da simulacao.",
          escrita_executada: false,
        }, 400, headers);
      }
      if (requestedConfirmation !== expectedConfirmation) {
    ...
    if (operationMode === "verificar_piloto" || operationMode === "verificar_lote") {
      ...
      if (!safePilotLimit) {
        blockers.push(batchMode
          ? "O lote deve conter de dois a cinco itens e coincidir com a janela ativa."
